                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm text-text-secondary mb-4">
                   ...
                   {hasWriteAccess && (
                    <div className="flex gap-2 mt-4 pt-4 border-t border-border-subtle">
                      {trip.status === 'Pending' && (
                        <button className="flex-1 bg-accent hover:bg-accent-hover text-white py-1.5 rounded text-sm font-medium transition-colors" onClick={() => handleDispatchTrip(trip.id)}>Dispatch</button>
                      )}
                      {trip.status === 'Dispatched' && (
                        <button className="flex-1 bg-success hover:bg-success/80 text-white py-1.5 rounded text-sm font-medium transition-colors" onClick={() => handleCompleteTrip(trip.id, v?.odometer || 0)}>Complete</button>
                      )}
